...
THRESHOLDS = {"rpm_max": 650, "force_max_kn": 14.0, "temp_max_c": 500}
...
def analyze_defect_event(df, event_index):
    ...
    if event["temperature_c"] > THRESHOLDS["temp_max_c"]:
        violations.append(
            f"Temperature {event['temperature_c']}C exceeds max {THRESHOLDS['temp_max_c']}C"
        )
...
def create_vectorstore(documents):
    ...
    vectorstore = Chroma.from_documents(
        documents=chunks, embedding=embeddings, persist_directory="./.chromadb_sensor"
    )
...
What is the root cause according to FSW standards?"""
    ...
    llm = OllamaLLM(model="llama3.2:1b", temperature=0, base_url="http://localhost:11434")
...
def main():
    ...
    while True:
        ...
        doctypes = [doc.metadata.get("doctype", "unknown") for doc in result["source_documents"]]
